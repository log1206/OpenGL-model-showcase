...
struct model_light
{
	...
	Vector3 direction;
	Vector3 diffusion;
	GLfloat exponent; //only spot light use
	GLfloat cutoff; //only spot light use
	GLfloat shininess = 64;
	GLfloat constant;
	GLfloat linear;
	GLfloat quadratic;
};
model_light direct_light;
model_light position_light;
model_light spot_light;

Vector3 cur_diffusion_intensity = Vector3(1, 1, 1);
...
LightMode cur_lig_mode = Directional;
...
FilterMode cur_min_mode = Near;
FilterMode cur_mag_mode = Near;
...
void RenderScene(int per_vertex_or_per_pixel) {	
	...
	glUniform1i(perModeLoc, per_vertex_or_per_pixel);
	...
	for (int i = 0; i < models[cur_idx].shapes.size(); i++) 
	{
		...
		ka[0] = models[cur_idx].shapes[i].material.Ka.x;
		ka[1] = models[cur_idx].shapes[i].material.Ka.y;
		ka[2] = models[cur_idx].shapes[i].material.Ka.z;

		kd[0] = models[cur_idx].shapes[i].material.Kd.x;
		kd[1] = models[cur_idx].shapes[i].material.Kd.y;
		kd[2] = models[cur_idx].shapes[i].material.Kd.z;


		ks[0] = models[cur_idx].shapes[i].material.Ks.x;
		ks[1] = models[cur_idx].shapes[i].material.Ks.y;
		ks[2] = models[cur_idx].shapes[i].material.Ks.z;

		glUniform3fv(LocKa, 1, ka);
		glUniform3fv(LocKd, 1, kd);
		glUniform3fv(LocKs, 1, ks);
		if (cur_lig_mode == Directional) {
			glUniform1i(modeLocLight, 1);
			glUniform1f(shiniLoc, direct_light.shininess);
			light_position[0] = direct_light.position.x;
			light_position[1] = direct_light.position.y;
			light_position[2] = direct_light.position.z;
			diffusion_intensity[0] = direct_light.diffusion.x;
			diffusion_intensity[1] = direct_light.diffusion.y;
			diffusion_intensity[2] = direct_light.diffusion.z;
		}
		else if (cur_lig_mode == Positional) {
			glUniform1i(modeLocLight, 2);
			glUniform1f(shiniLoc, position_light.shininess);
			light_position[0] = position_light.position.x;
			light_position[1] = position_light.position.y;
			light_position[2] = position_light.position.z;
			diffusion_intensity[0] = position_light.diffusion.x;
			diffusion_intensity[1] = position_light.diffusion.y;
			diffusion_intensity[2] = position_light.diffusion.z;
		}
		else if (cur_lig_mode == Spot) {
			glUniform1i(modeLocLight, 3);
			glUniform1f(shiniLoc, spot_light.shininess);
			glUniform1f(cutLoc, spot_light.cutoff);	
			light_position[0] = spot_light.position.x;
			light_position[1] = spot_light.position.y;
			light_position[2] = spot_light.position.z;
			diffusion_intensity[0] = spot_light.diffusion.x;
			diffusion_intensity[1] = spot_light.diffusion.y;
			diffusion_intensity[2] = spot_light.diffusion.z;
		}

		glUniform3fv(lightPosLoc, 1, light_position);
		glUniform3fv(diffusLoc, 1, diffusion_intensity);
		...
		if(cur_mag_mode == Linear)
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
		else
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);

		if(cur_min_mode == Linear) 
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);
		else
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
		

		if (cur_re_mode == Repeat) {
...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	if (action == GLFW_PRESS) {
		...
		case GLFW_KEY_U:
			...
			break;
		case GLFW_KEY_G:
			if (cur_mag_mode == Near)
				cur_mag_mode = Linear;
			else
				cur_mag_mode = Near;
			break;
		case GLFW_KEY_B:
			if (cur_min_mode == Near)
				cur_min_mode = Linear;
			else
				cur_min_mode = Near;
			break;
		case GLFW_KEY_V:
			if (cur_re_mode == Mirror)
				cur_re_mode = Repeat;
			else
				cur_re_mode = Mirror;
			break;
		case GLFW_KEY_I:
			cout << "current magnification: " << ((cur_mag_mode == Near) ? "Nearnest" : "Linear" )<< endl;
			cout << "current minification: " << ((cur_min_mode == Near) ? "Nearnest" : "Linear_minimap_Linear") << endl;
			cout << "current repeat/mirror mode: " << ((cur_re_mode == Repeat) ? "Repeat" : "Mirror_repeat") << endl;
			cout << endl;
			break;
		case GLFW_KEY_K:
		...
		case GLFW_KEY_L:
			switch (cur_lig_mode) {
				case Directional:
					cur_lig_mode = Positional;
					break;
				case Positional:
					cur_lig_mode = Spot;
					break;
				case Spot:
					cur_lig_mode = Directional;
					break;
				}
			break;
...
void scroll_callback(GLFWwindow* window, double xoffset, double yoffset)
{
	...
	case Light:
		switch (cur_lig_mode) {
		case Directional:
			direct_light.diffusion.x += yoffset / 100;
			direct_light.diffusion.y += yoffset / 100;
			direct_light.diffusion.z += yoffset / 100;
			break;
		case Positional:
			position_light.diffusion.x += yoffset / 100;
			position_light.diffusion.y += yoffset / 100;
			position_light.diffusion.z += yoffset / 100;
			break;
		case Spot:
			spot_light.cutoff += yoffset / 5;
	...
	case Shininess:
		switch (cur_lig_mode) {
		case Directional:
			if (direct_light.shininess > 0)
				direct_light.shininess -= yoffset;

			break;
		case Positional:
			if (position_light.shininess > 0)
				position_light.shininess -= yoffset;

			break;
		case Spot:
			if (spot_light.shininess > 0)
				spot_light.shininess -= yoffset;
			break;
		}
		break;
	}
		...
		if (starting_press_x < 0 || starting_press_y < 0) {
			...
			case Light:
				...
				case Directional:
					...
					direct_light.position.x = starting_press_x + xpos / 100;
					direct_light.position.y = starting_press_y - ypos / 100;
					break;
				case Positional:
					position_light.position.x = starting_press_x + xpos / 100;
					position_light.position.y = starting_press_y - ypos / 100;

					break;
				case Spot:
					spot_light.position.x = starting_press_x + xpos / 100;
					spot_light.position.y = starting_press_y - ypos / 100;
					break;
				}
				break;
			}
...
void initParameter()
{
	...
	direct_light.constant = 0.01;
	direct_light.linear = 0.8;
	direct_light.quadratic = 0.1;
	direct_light.position = Vector3(1, 1, 1);
	direct_light.direction = Vector3(0, 0, 0);
	direct_light.diffusion = Vector3(1, 1, 1);



	spot_light.constant = 0.05;
	spot_light.linear = 0.3;
	spot_light.quadratic = 0.6;
	spot_light.position = Vector3(0, 0, 2);
	spot_light.cutoff = 15;
	spot_light.exponent = 50;
	spot_light.diffusion = Vector3(1, 1, 1);

	position_light.position = Vector3(0, 2, 1);
	position_light.diffusion = Vector3(1, 1, 1);
}
...
void setUniformVariables()
{
	...
	LocKs = glGetUniformLocation(program, "ks");
	perModeLoc = glGetUniformLocation(program, "permode");


	modeLocLight = glGetUniformLocation(program, "light_mode");
	shiniLoc = glGetUniformLocation(program, "shininess");
	diffusLoc = glGetUniformLocation(program, "diffusion_intensity");
	lightPosLoc = glGetUniformLocation(program, "light_position");
	cutLoc = glGetUniformLocation(program, "cutoff");
	// [TODO] Get uniform location of texture

	iLocTex = glGetUniformLocation(program, "text");
}
